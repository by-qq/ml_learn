...
def parse_mnist(image_filename, label_filename):
    ...
    ### 你的代码开始
    # 根据扩展名决定是否要解压
    open_label = gzip.open if label_filename.endswith('.gz') else open
    open_image = gzip.open if image_filename.endswith('.gz') else open

    with open_image(image_filename, 'rb') as f:
        # 读取图像数量、行数、列数
        ...
        if magic_number == 2051:
            ...
            # 重塑为(num_images, rows, cols)的形状
            images = images.reshape(num_images, rows * cols) / 255.0

    with open_label(label_filename, 'rb') as f:
        # 读取解析MNIST文件格式的关键部分
        # f.read(4)：从文件中读取4个字节，
        # struct.unpack('>I', ...)：解析字节数据，>大端字节序，I表示无符号整数
        # [0]获取解析结果的第一个元素
